...
# Import Data
df <- read.csv("Data/bookings.csv")

# Change to Standard Column Names
colnames(df) <- c("year", "month", "plant_ID", "customer_ID", "market_code", "material_type_ID", "booked_tons")
...
df %>%
  mutate(
    ...
    month_num = ifelse(month == "January", 1,
  ...
  write.csv("Cleaned_Data/clean_bookings.csv")
